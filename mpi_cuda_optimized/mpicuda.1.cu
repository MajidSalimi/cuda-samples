   ...
   if (i<N) A[i] = A[i] + 1; 
    ...
    int main(int argc, char **argv){
      ...
      // Get processor id
	    ...
	    if (myid == 0)  printf("The size of data: %d  Bytes\n ", nBytes);
		...
		{  
			...
			if (myid == 0) {
				...
				t_end = MPI_Wtime();
				printf("Iteration: %d , Total Time: %3f  (ms) \n", iter , ((t_end - t_start)* 1e3));
				...
				cudaMallocHost((void **)&myarr, k*sizeof(int));
				block_size=32;
				grid_size = 512;
				...
				//cudaMallocHost((void **)&a_d, k*sizeof(int));
				...
				cudaMemcpy(myarr,a_d,k*sizeof(int),cudaMemcpyDeviceToHost);
				...
				MPI_Send(myarr, k, MPI_INT, 0, myid, MPI_COMM_WORLD);   
